...
def test_discipline_stacking():
    ...
    assert not no2 and "Pyramiding" in reason2, reason2
    three = {"trades": [{"status": "LIVE", "direction": "BUY"} for _ in range(3)],
             "history": []}
    no3, reason3 = pt.discipline_check(three, "BUY", 0.01, sl, entry, atr)
    assert not no3 and "same-direction" in reason3, reason3
